...
namespace FriendsInNeed
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var streets = ReadWeightedGraph(parameters[0], parameters[1]);

            var result = int.MaxValue;
            hospitals.ForEach(x =>
            {
                var curr = Dijkstra(x, streets, hospitals);
                if (curr < result)
                {
                    result = curr;
                }
        ...
        private static List<Node>[] ReadWeightedGraph(int nodesCount, int streetsCount)
        {

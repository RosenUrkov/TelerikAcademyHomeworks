...
namespace Dijkstra
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var startNode = int.Parse(startAndEnd[0]);
            var endNode = int.Parse(startAndEnd[1]);

            var middleNodes = Console.ReadLine().Split(' ');
            var firstMiddle = int.Parse(middleNodes[0]);
            var secondMiddle = int.Parse(middleNodes[1]);

            var graph = ReadWeightedGraph(parameters[0], parameters[1]);

            var firstMiddleDistances = Dijkstra(firstMiddle, graph);
            var secondMiddleDistances = Dijkstra(secondMiddle, graph);

            var result1 = firstMiddleDistances[startNode] + firstMiddleDistances[secondMiddle] + secondMiddleDistances[endNode];
            var result2 = secondMiddleDistances[startNode] + secondMiddleDistances[firstMiddle] + firstMiddleDistances[endNode];
